...
- **Mounting / layout:** the picker fills its parent (`h-full`) and its two panes scroll internally, so mount it in a **height-bounded flex parent** — `flex-1 flex min-h-0` inside a `flex flex-col max-h-[88vh]` modal. The panes carry `min-h-0` themselves so their `overflow-y-auto` lists scroll rather than growing to content height; **don't strip it** — a missing `min-h-0` on a flex child whose `min-height` defaults to `auto` makes a long WO list overflow and clip behind the footer instead of scrolling (the S61 `aede010` fix).
