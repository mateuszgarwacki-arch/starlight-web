...
### Addendum: Job page layout pass (same day)

**Context:** User screenshot showed the job detail page getting crowded — 5 stacked panels (PM Queries, Learnings, Invoices, Orders, Job Documents) between the stats strip and the quote lines table meant constant scrolling. Filter state also reset to "To Do" on every back-from-scope navigation. No scope status visible on quote-line rows.

**Shipped (4 changes):**

1. **Filter persistence per-job.** `activeFilter` now lazy-inits from `sessionStorage.getItem('job-filter-${jobId}')` and saves on change via useEffect. Session-scoped (dies on tab close — filter preference shouldn't leak across days). Per-job key so different jobs remember independent filter states.

2. **Scope status pill on quote-line rows.** The Scope column previously showed just a FileText icon as a link to the scope page. Replaced with a coloured `StatusBadge` pill that IS the link — the pill text is the status, the pill click opens the scope. Labels shortened for workshop readability: "Workshop Completed" → "Built", "Cancelled-Cost-Retained" → "Cancelled". Other values ("Active", "Provisional") shown verbatim.

3. **Scope statuses added to `statusClass()`** in `src/lib/utils.ts`. Mapped onto the existing WO colour palette — no new CSS classes needed. "Active" → `badge-in-progress` (blue), "Provisional" → `badge-on-hold` (amber), "Workshop Completed" → `badge-complete` (green), "Cancelled-Cost-Retained" → `badge-voided` (grey).

4. **Toolbar row replaces 5 stacked panels.** New horizontal button row with 5 tabs — PM Queries, Orders, Invoices, Documents, Learnings (left-to-right: workflow → financial → reference → reflective). Click a button → that panel expands full-width below. Click again → collapses. Click another → swaps. Default state: all collapsed, nothing below the buttons, so quote lines sit right under Cost Analysis. Cost Analysis itself stays as a full-width panel above the toolbar (unchanged per user request).

   Each button shows an at-a-glance badge computed from `rpc_job_header_counts(p_job_id)`:
   - PM Queries: count of open (amber warning)
   - Orders: "N out" count of needs_ordering BOM rows not yet ordered (amber warning)
   - Invoices: "£X unalloc" when `unallocated_total > 0` (amber warning), else just count
   - Documents: count (no warning)
   - Learnings: "N open" where open = `actionable AND NOT resolved` (amber warning)

   Badge labels and warning pills let user see the state without clicking. Clicking is only needed to act on the data.

5. **Panel components made toolbar-aware.** `PmQueriesJobPanel` gained `defaultExpanded` prop, `JobInvoicesPanel` and `JobOrdersPanel` gained `defaultCollapsed` prop. Default values preserve existing behaviour everywhere else; toolbar passes the opposite so the panel renders open when selected. `LearningsSection` already supported `defaultCollapsed`. `WODocumentsPanel` has no collapse chrome — renders content directly.

**SQL run (S36c):**
- New RPC `rpc_job_header_counts(p_job_id INTEGER) RETURNS json SECURITY INVOKER STABLE` — returns 8 counts/totals in one call. Reuses existing `qry_invoice_job_rollup` view for invoice totals. Runs in parallel with `rpc_job_detail_data` via `Promise.all` in `loadData`, so zero extra latency.

**Files changed:**
| File | Change |
|---|---|
| `src/app/(dashboard)/jobs/[id]/page.tsx` | +180/-40: new state (`activeTool`, `headerCounts`), lazy-init of `activeFilter` from sessionStorage, save effect, `displayScopeStatus` helper, scope column rewrite, toolbar JSX replacing the 5-panel section |
| `src/lib/utils.ts` | +4 lines: 4 scope status cases added to `statusClass()` switch |
| `src/components/pm-queries-job-panel.tsx` | `defaultExpanded?: boolean = false` prop added |
| `src/components/job-invoices-panel.tsx` | `defaultCollapsed?: boolean = true` prop added |
| `src/components/job-orders-panel.tsx` | `defaultCollapsed?: boolean = true` prop added |

**Conventions added (S36c):**
- **Per-resource session-scoped UI state in sessionStorage, not localStorage.** Filter choice per job = session-scoped because it's workflow state, not a long-term preference. Uses key pattern `job-filter-${jobId}`. Different from the mobile-tasks-collapsed state (S36) which is `localStorage` because that IS a long-term preference. Rule of thumb: "did I finish a task on this page" state = session. "How I like to see this page" preference = local.
- **Dedicated lightweight RPC for at-a-glance counts, separate from the main data RPC.** `rpc_job_header_counts` could have been folded into `rpc_job_detail_data` but keeping it separate lets it be cached independently, called from other surfaces (e.g. a future job list view), and revised without risk to the main RPC. Runs in parallel via `Promise.all` — no latency cost.
- **Coloured status pills that ARE the link,** rather than icon-link + separate status badge. One element, two purposes. Keeps the row compact and reinforces that clicking a status jumps to the thing that has that status.
- **Tool toolbar pattern for grouped secondary surfaces.** When a page has 4+ secondary panels (reviews, notes, financials, references), a horizontal toolbar row with click-to-expand is a better default than stacked accordions — the at-a-glance signal lives in the button badge, and the user pays the vertical cost only for the one they actually want to see. Reusable pattern for other heavy pages if they emerge.
- **Scope-status short labels on crowded surfaces.** DB stores "Workshop Completed" (18 chars) for a good reason — it's unambiguous. But on a narrow table column "Built" (5 chars) is more scannable. Keep the DB value long and clear; abbreviate only at display time via a local helper function on the specific surface that needs it. Don't push display abbreviations back into the schema.

**Deferred / next-pass candidates:**
- **Default tool open from URL query param** — `?tool=orders` could deep-link to a specific expanded panel. Useful if we add cross-links from other pages ("view this job's invoices").
- **Kill the panel's internal collapse chrome** when rendered inside the toolbar. Currently the `PmQueriesJobPanel` / invoices / orders panels still render their own "X (N open) [chevron]" header inside the toolbar's expanded area — works fine but slightly redundant. Could add a `hideHeader` prop to collapse the duplication if it bothers anyone in practice.
- **Per-user default tool** — admin/PM types often use different panels. Could remember the last-opened tool per user in `localStorage` and default to it. Only worth doing if a clear preference pattern emerges.


---

## Session 37 — 22 April 2026

### Summary
Rock-solid fix for BOM unit_cost / timber costing. The `standard_length/1000` multiplier was being applied **three times**: cut-list extractor wrote per-length cost at creation, the view `qry_bom_enriched` multiplied again, and the frontend `bomRowCost` helper multiplied a third time. Timber totals displayed at ~4.8× their correct value. User flagged this as "already changed 3 times and again — fucked". This session locks in a single invariant and enforces it at every write path and every read path.

### The invariant (documented in schema + conventions)

**`tbl_wo_bom.unit_cost` is the price for ONE of `tbl_wo_bom.unit`. Line total = `quantity × unit_cost`. No multipliers at read time. Any write path that changes the stored unit must convert the stored cost in the same write.**

Same invariant for `actual_unit_cost` (populated at invoice reconciliation).

### Data audit before the fix

- 20 Length-unit timber rows stored as Convention B (per-length cost, correct)
- 6 rows stored as Convention A (per-metre cost — legacy from earlier code that didn't convert)
- 0 custom-priced rows

The 6 Convention A rows were still being created as recently as **today** (bom_id 182 stored ~18 minutes before the user's screenshot) because the scope-level "+ Add Material" flow had a TODO comment in place of the metre→length conversion.

### SQL migration (`s37_fix_bom_unit_cost_invariant`)

Applied via Supabase MCP. All transactional, rolls back together on any error.

1. **Normalise the 6 legacy rows** to Convention B:
   ```sql
   UPDATE tbl_wo_bom SET unit_cost = ROUND((unit_cost * (m.standard_length/1000.0))::numeric, 2)
   FROM tbl_materials m
   WHERE m.material_id = tbl_wo_bom.material_id
     AND LOWER(unit) = 'length'
     AND LOWER(m.unit) IN ('metre','meter','m')
     AND m.standard_length > 0
     AND ABS(tbl_wo_bom.unit_cost - m.current_unit_cost) < 0.05;
   ```
   Idempotent — after the update, `unit_cost` no longer equals `current_unit_cost`, so re-running the migration is a no-op. Safe.

2. **Rebuild `qry_bom_enriched`** — `planned_line_cost = qty × unit_cost`, `reconciled_line_cost = qty × (actual_unit_cost ?? unit_cost)`. Kept the `unit_to_base_multiplier` column (always `1.0` now) for backward compatibility with any dashboards still reading it. Column list unchanged so `CREATE OR REPLACE VIEW` works without needing CASCADE.

3. **Rebuild `rpc_pm_job_overview`** — same multiplier removed from the `bom_raw` CTE. The `bom_grouped` total_cost sum is now just `COALESCE(actual ?? planned, 0) × qty`.

4. **Schema comments** on `tbl_wo_bom.unit_cost` and `tbl_wo_bom.actual_unit_cost` documenting the invariant and citing S37.

### Verification

- `qry_bom_enriched` for bom_id 183 (3x1 PAR, 26m actual, 6× 4.8m): `planned_line_cost = £31.68` (was £152.06 — a 4.8× reduction, matches the one-multiplier removed).
- Grosvenor (job 8) scope totals: net reduction of ~£1,500 in planned materials, concentrated on scopes containing timber.
- All 6 legacy rows now show stored `unit_cost = current_unit_cost × (standard_length/1000)` exactly — Convention B throughout the database.

### Frontend (`src/components/work-orders-panel.tsx`)

Three edits in one file:

1. **`bomRowCost` helper** — dropped the multiplier logic entirely. Now: `return qty * (actual_unit_cost ?? unit_cost)`. No longer needs the `materials` closure for cost math. Same comment block documents the S37 invariant.

2. **`confirmScopeMaterial`** — finished the `// Would need standard_length — skip for now, handle in Phase B` TODO. When `catUnit === "metre" && buyUnit === "length" && standard_length`, multiply `unitCost` by `standard_length/1000` so the stored cost is per-length, matching the stored `unit = "Length"`. This is the fix that stops new Convention A rows from ever being created again.

3. **Scope material preview** (in the dialog render block) — matching conversion so the preview total agrees with what gets saved on confirm. Previously these two could disagree by 4.8× for timber.

4. **Types** — added `standard_length: number | null` to `scopeMatResults` and `scopeMatSelected` state types so TypeScript compiles.

### Why this happened (and how to prevent the next one)

The root cause wasn't a single bug — it was **competing conventions with no invariant enforcement**. Different write paths used different conventions; read paths tried to "normalise" at read time but assumed one convention; data drifted. Every "fix" picked a side without forcing the other side to agree.

The durable fix is the invariant itself, not any single code change: **stored `unit_cost` is denominated in the stored `unit`. Full stop.** With that rule written into the schema comment and documented in `05_conventions.md`, any future write path that breaks it is a trivially-spottable bug — and the read side never needs clever logic.

### Files changed

| File | Change |
|---|---|
| `qry_bom_enriched` (view) | Rebuilt without the `standard_length/1000` CASE in `planned_line_cost` / `reconciled_line_cost`. `unit_to_base_multiplier` column kept as constant 1.0 for backward compat. |
| `rpc_pm_job_overview` (function) | `bom_raw` CTE simplified. `bom_grouped.total_cost` is now qty × cost. |
| `tbl_wo_bom.unit_cost` + `actual_unit_cost` | Column comments added. |
| `src/components/work-orders-panel.tsx` | `bomRowCost` simplified; `confirmScopeMaterial` TODO closed; preview conversion added; state types extended. |

### Conventions added (S37)

- **BOM unit_cost invariant (locked)**: `tbl_wo_bom.unit_cost` is price for ONE of `tbl_wo_bom.unit`. Line total is always `quantity × unit_cost`. No read-time multipliers. Write paths own conversion when they change the stored unit. Applies equally to `actual_unit_cost`.
- **One invariant beats three patches**: when the same bug has been "fixed" multiple times, the fix is often a missing invariant, not a missing check. Name the invariant, document it in the schema (comment on the column), enforce it at every write path. Read paths then become trivial and can't drift.
- **Audit data before data-migrating**: before normalising "bad" rows, classify them into precise buckets (Convention A / B / custom) using maths that distinguishes the conventions numerically (`ABS(cost - per_metre) < 0.05` here). Confirms the migration predicate is tight enough not to catch rows that just happen to match by coincidence.
- **`CREATE OR REPLACE VIEW` over DROP CASCADE** when the column list is unchanged. Avoids dropping and recreating 5 dependent views for a logic-only change.
