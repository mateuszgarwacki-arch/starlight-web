    ...
    if (!sessionStarted) {
      return (
        <div className="space-y-4">
          ...
          {selectedAsset.description && <p className="text-xs text-gray-500 bg-white rounded-xl border border-gray-200 px-4 py-3">{selectedAsset.description}</p>}
          {/* Photos */}
          {photos.length > 0 && (
            <>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {photos.map(p => (
                    <div key={p.photo_id} className="shrink-0 w-32">
                      {p.url ? <a href={p.url} target="_blank" rel="noopener noreferrer"><img src={p.url} alt={p.caption || ""} className="w-32 h-24 object-cover rounded-lg border border-gray-200" /></a>
          ...
          <div className="bg-white rounded-xl border border-gray-200 px-4 py-3">
            ...
            {tasks.slice(0, 5).map(t => (
              <div key={t.task_id} className="flex items-center gap-2 py-1 text-xs text-gray-500">
                <span className={`w-1.5 h-1.5 rounded-full ${dueDot(t.due_status)}`} />
                <span className="flex-1">{t.description}</span>
                <span className={t.due_status === "overdue" ? "text-red-500 text-[10px] font-medium" : "text-[10px] text-gray-400"}>{dueLabel(t.due_status)}</span>
              </div>
            ))}
            {tasks.length > 5 && <p className="text-[10px] text-gray-400 mt-1">+ {tasks.length - 5} more</p>}
          </div>
          {/* Start button */}
          <button onClick={startSession}
            className="w-full py-4 bg-starlight-blue text-white font-semibold rounded-xl hover:bg-blue-700 active:bg-blue-800 transition-colors flex items-center justify-center gap-2 shadow-lg">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => { if (confirm("Abandon this maintenance session?")) { if (timerRef.current) clearInterval(timerRef.current); setSelectedAsset(null); setSessionStarted(false); } }}
            className="p-1 text-gray-400"><ChevronLeft className="h-5 w-5" /></button>
          <div className="flex-1">
        ...
        {photos.length > 0 && (
          <>
            <button onClick={() => setShowPhotos(!showPhotos)}
              className="w-full flex items-center justify-between px-3 py-2 bg-white rounded-lg border border-gray-200 text-left">
              <span className="text-[11px] text-gray-500 flex items-center gap-1"><Image className="h-3 w-3" />Photos ({photos.length})</span>
              <ChevronDown className={`h-3.5 w-3.5 text-gray-400 transition-transform ${showPhotos ? "rotate-180" : ""}`} />
            </button>
            {showPhotos && (
                ...
                {photos.map(p => (
                  <div key={p.photo_id} className="shrink-0">
                    {p.url ? <a href={p.url} target="_blank" rel="noopener noreferrer"><img src={p.url} alt="" className="w-28 h-20 object-cover rounded-lg border" /></a>
                      : <div className="w-28 h-20 bg-gray-100 rounded-lg" />}
                    {p.caption && <p className="text-[8px] text-gray-400 mt-0.5 w-28 truncate">{p.caption}</p>}
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="bg-white rounded-lg border border-gray-200 px-3 py-2">
          ...
          <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-green-500 rounded-full transition-all duration-300" style={{ width: `${tasks.length > 0 ? (checkedCount / tasks.length) * 100 : 0}%` }} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="divide-y divide-gray-100">
            {tasks.map((task, i) => {
              ...
              return (
                <div key={task.task_id} className={`px-4 py-3 transition-colors ${check.checked ? "bg-green-50/30" : ""}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {task.instructions && !check.checked && <p className="text-[10px] text-gray-400 mt-0.5">{task.instructions}</p>}
                    ...
                    <button onClick={() => toggleFlag(task.task_id)}
                      className={`p-2 rounded-lg transition-colors shrink-0 ${check.flagged ? "bg-red-50 text-red-500" : "text-gray-300"}`}>
                  ...
                  {check.checked && (
                    <input value={check.note} onChange={e => setCheckNote(task.task_id, e.target.value)}
                      ...
                      className="mt-2 ml-9 w-[calc(100%-2.25rem)] text-xs px-3 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-starlight-blue bg-gray-50" />
                  )}
                  {check.flagged && (
                    <div className="mt-2 ml-9 space-y-1.5">
                      <input value={check.flagTitle} onChange={e => setFlagTitle(task.task_id, e.target.value)}
                        placeholder="What's the issue? *"
                        className="w-[calc(100%-2.25rem)] text-xs px-3 py-1.5 border border-red-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-400 bg-red-50/30" />
        ...
        <button onClick={completeMaintenance} disabled={submitting || checkedCount === 0}
          className="w-full py-3.5 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 active:bg-green-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg">
