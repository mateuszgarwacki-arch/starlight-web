...
import { Check, ChevronDown, ChevronUp, X, Plus, Trash2 } from "lucide-react";
...
export default function MobileSchedule() {
  ...
  const [unavailMonth, setUnavailMonth] = useState(() => {
    const d = new Date();
  ...
  const loadData = useCallback(async () => {
    ...
    // Fetch all future schedule rows for this freelancer
    ...
    // Separate unavailable vs bookings
    const unavailRows = allRows.filter((r) => r.status === "Unavailable");
    const bookingRows = allRows.filter((r) => r.status !== "Unavailable");
    setUnavailable(unavailRows);

    // Get unique job IDs
    const jobIds = [...new Set(bookingRows.map((r) => r.job_id).filter(Boolean))] as number[];
    let jobMap: Record<number, JobInfo> = {};
    if (jobIds.length > 0) {
    ...
    // Group by booking_group (or individual rows without a group)
    ...
    const bookingGroups: BookingGroup[] = Object.entries(groupMap).map(([key, gRows]) => {
      ...
      return {
        ...
        job: firstRow.job_id ? jobMap[firstRow.job_id] || null : null,
  ...
  // ============================================================
  // Confirm with exceptions (from day toggles)
  // ============================================================
  const confirmWithExceptions = async (group: BookingGroup) => {
    ...
    group.rows.forEach((r) => {
      if (dayToggles[r.schedule_id] === false) {
        declineIds.push(r.schedule_id);
      } else {
        confirmIds.push(r.schedule_id);
      }
    });
    if (confirmIds.length > 0) {
      await supabase
        .from("tbl_freelancer_schedule")
        .update({ status: "Confirmed" })
        .in("schedule_id", confirmIds);
    }
    if (declineIds.length > 0) {
      await supabase
        .from("tbl_freelancer_schedule")
        .update({ status: "Declined" })
        .in("schedule_id", declineIds);
    }
  ...
  // ============================================================
  // Expand group to show day toggles
  // ============================================================
  ...
  // Month calendar for unavailability picker
  const getMonthDays = (ym: string) => {
    const [y, m] = ym.split("-").map(Number);
    const first = new Date(y, m - 1, 1);
    const last = new Date(y, m, 0);
    const days: { date: string; dayOfWeek: number; dayNum: number; isWeekend: boolean }[] = [];
    for (let d = 1; d <= last.getDate(); d++) {
      const dt = new Date(y, m - 1, d);
      const ds = dt.toISOString().split("T")[0];
      days.push({ date: ds, dayOfWeek: dt.getDay(), dayNum: d, isWeekend: dt.getDay() === 0 || dt.getDay() === 6 });
    }
    return { days, firstDow: first.getDay(), monthLabel: first.toLocaleDateString("en-GB", { month: "long", year: "numeric" }) };
  };

  const shiftMonth = (dir: number) => {
    const [y, m] = unavailMonth.split("-").map(Number);
    const nd = new Date(y, m - 1 + dir, 1);
    setUnavailMonth(`${nd.getFullYear()}-${String(nd.getMonth() + 1).padStart(2, "0")}`);
  };

  const { days: monthDays, firstDow, monthLabel } = getMonthDays(unavailMonth);
  // Pad start: Monday=1 based grid
    ...
    <div className="space-y-4">
      ...
      {pendingCount > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
          <p className="text-sm font-medium text-amber-800">{pendingCount} booking{pendingCount > 1 ? "s" : ""} need{pendingCount === 1 ? "s" : ""} your response</p>
        </div>
      )}
      ...
      {groups.map((g) => {
        ...
        const confirmedCount = isExpanded
          ? Object.values(dayToggles).filter(Boolean).length
          : 0;
        const declinedCount = isExpanded
          ? Object.values(dayToggles).filter((v) => !v).length
          : 0;
        ...
        return (
          <div
            ...
            className={
              ...
              (isPending ? "border-2 border-amber-300" : "border border-gray-100")
            }
          >
            {/* Card header */}
              ...
              <div className="flex justify-between items-start mb-1">
                <div className="flex-1 min-w-0">
                  <p className="text-[15px] font-semibold text-navy truncate">
                    {g.job?.job_name || "General workshop"}
                  </p>
                ...
                <span className={
                  ...
                  (isPending ? "bg-amber-100 text-amber-700" :
                   ...
                   g.status === "declined" ? "bg-red-100 text-red-700" :
                   "bg-blue-100 text-blue-700")
                }>
                  {isPending ? "Pending" : isConfirmed ? "Confirmed" : g.status === "declined" ? "Declined" : "Partial"}
                </span>
              </div>
            ...
            {isExpanded && (
              <div className="px-4 pb-4">
                  ...
                  {g.rows.map((r) => {
                    ...
                    return (
                      <button
                        ...
                        className={
                          "w-full flex justify-between items-center px-3 py-2.5 rounded-lg transition-colors " +
                          (isOn ? "bg-green-50" : "bg-red-50")
                        }
                      >
                        <span className={
                          "text-[13px] font-medium " + (isOn ? "text-green-700" : "text-red-600")
                        }>
                ...
                <button
                  onClick={() => confirmWithExceptions(g)}
                  disabled={acting || confirmedCount === 0}
                ...
                >
                  {declinedCount > 0
                    ? `Confirm ${confirmedCount} day${confirmedCount > 1 ? "s" : ""}, decline ${declinedCount}`
                    : `Confirm all ${confirmedCount} days`}
                </button>
                <button
                  onClick={() => { setExpandedGroup(null); setDayToggles({}); }}
                  className="w-full py-2.5 text-gray-500 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
                >
                  Back
                </button>
      ...
      <div className="mt-6">
        <h2 className="text-sm font-semibold text-navy mb-3">My availability</h2>
              ...
              <div key={u.schedule_id} className="flex items-center justify-between bg-white rounded-xl border border-gray-100 px-4 py-3">
                <div>
                  ...
                  {u.unavailable_reason && (
                    <p className="text-[11px] text-gray-400">{u.unavailable_reason}</p>
                  )}
                </div>
                <button
                  onClick={() => removeUnavail(u.schedule_id)}
                  disabled={acting}
        ...
        <button
          onClick={() => { setShowUnavailForm(true); setUnavailDates([]); setUnavailReason(""); }}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-end z-50">
          <div className="bg-white w-full rounded-t-2xl max-h-[85vh] overflow-y-auto">
            <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
              ...
              <button onClick={() => setShowUnavailForm(false)} className="p-1 text-gray-400">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="px-5 py-4">
              ...
              <div className="flex items-center justify-between mb-3">
                <button onClick={() => shiftMonth(-1)} className="px-3 py-1.5 text-gray-400 hover:text-navy text-sm">‹</button>
              ...
              <div className="grid grid-cols-7 gap-1 mb-4">
                ...
                {monthDays.map((d) => {
                  ...
                  const alreadyUnavail = unavailable.some((u) => u.scheduled_date === d.date);
                  const hasBooking = groups.some((g) => g.rows.some((r) => r.scheduled_date === d.date && r.status !== "Declined"));
                  const disabled = isPast || alreadyUnavail || hasBooking;
                  return (
                    <button
                      key={d.date}
                      onClick={() => !disabled && toggleUnavailDate(d.date)}
                      disabled={disabled}
                      className={
                        "text-center py-2.5 rounded-lg text-[13px] transition-colors " +
                        (isSelected ? "bg-red-100 text-red-700 font-semibold" :
                         alreadyUnavail ? "bg-gray-100 text-gray-300 line-through" :
                         hasBooking ? "bg-green-50 text-green-600" :
                         isPast ? "text-gray-200" :
                         d.isWeekend ? "text-gray-300" :
              ...
              <button
                ...
                disabled={acting || unavailDates.length === 0}
