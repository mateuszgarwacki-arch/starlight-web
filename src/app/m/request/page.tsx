...
const CATEGORIES = [
  ...
  { value: "restock", label: "Restock", icon: Archive, color: "bg-phase-2/15 text-phase-2 border-phase-2/30" },
  { value: "safety", label: "Safety", icon: AlertTriangle, color: "bg-starlight-red/10 text-starlight-red border-starlight-red/30" },
  { value: "general", label: "General", icon: MessageSquare, color: "bg-surface-mid text-muted border-subtle" },
];
...
export default function MobileRequestPage() {
  ...
  return (
    <div className="space-y-5 max-w-lg">
      ...
      <div>
        <label className="text-xs font-medium text-muted mb-1.5 block">What&apos;s needed?</label>
        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. 18mm MDF running low" className="w-full px-4 py-3 bg-surface border border-subtle rounded-xl text-sm text-navy placeholder:text-faint focus:outline-none focus:ring-2 focus:ring-starlight-blue/30" autoFocus />
      </div>
      <div>
        <label className="text-xs font-medium text-muted mb-1.5 block">Category</label>
        <div className="flex flex-wrap gap-2">
          {CATEGORIES.map((cat) => { const Icon = cat.icon; return (<button key={cat.value} onClick={() => setCategory(cat.value)} className={"flex items-center gap-1.5 px-3.5 py-2 rounded-full text-xs font-medium border transition-all " + (category === cat.value ? cat.color + " ring-2 ring-offset-1 ring-current" : "bg-surface-dim text-muted border-subtle")}><Icon className="h-3.5 w-3.5" />{cat.label}</button>); })}
        </div>
      </div>
      <div>
        <label className="text-xs font-medium text-muted mb-1.5 block">Urgency</label>
        <div className="flex bg-surface rounded-xl border border-subtle overflow-hidden">
          <button onClick={() => setUrgency("normal")} className={"flex-1 py-2.5 text-xs font-medium transition-colors " + (urgency === "normal" ? "bg-navy text-white" : "text-muted")}>Normal</button>
          <button onClick={() => setUrgency("urgent")} className={"flex-1 py-2.5 text-xs font-medium transition-colors " + (urgency === "urgent" ? "bg-starlight-red text-white" : "text-muted")}>Urgent</button>
        </div>
      </div>
      <div>
        <label className="text-xs font-medium text-muted mb-1.5 block">Related job (optional)</label>
        {selectedJob ? (
      ...
      <div>
        <label className="text-xs font-medium text-muted mb-1.5 block">Details (optional)</label>
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Any extra detail..." rows={2} className="w-full px-4 py-3 bg-surface border border-subtle rounded-xl text-sm text-navy placeholder:text-faint focus:outline-none focus:ring-2 focus:ring-starlight-blue/30 resize-none" />
      </div>
      <div>
        <label className="text-xs font-medium text-muted mb-1.5 block">Photo (optional)</label>
        {photoPreview ? (
          <div className="relative">
            <img src={photoPreview} alt="Request photo" className="w-full h-40 object-cover rounded-xl border border-subtle" />
          ...
          <label className="flex items-center justify-center gap-2 w-full py-4 bg-surface border-2 border-dashed border-subtle rounded-xl text-muted text-sm cursor-pointer active:bg-surface-dim">
