...
import { ArrowLeft, Clock, Timer, ChevronRight, QrCode, X, Search } from "lucide-react";
...
interface RecentTask { task_id: number; title: string; category: string; hours: number | null; worked_date: string | null; status: string; created_at: string; job_name?: string | null; job_number?: string | null; }
...
export default function MobileTaskPage() {
      ...
      await scanner.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } },
        (decodedText: string) => {
          ...
          if (match) {
            ...
            if (wo) { setSelectedWo(wo); setTitle(`${wo.scope_name} — ${wo.description || "WO"}`); toast.success(`WO found: ${wo.scope_name}`); }
            else toast.error("WO not found or not active");
          } else toast.error("Not a valid WO QR code");
          stopScanner();
        }, () => {});
    } catch { toast.error("Camera access denied"); setShowScanner(false); }
  };
  const stopScanner = () => { scannerRef.current?.stop().catch(() => {}); scannerRef.current = null; setShowScanner(false); };

  const handleQuickLog = async (data: LogSheetData) => {
    ...
    const insertData: any = {
      freelancer_id: myId, title: title.trim(), description: data.notes || null,
      category: category === "task" ? "job_work" : category,
      job_id: category === "task" ? selectedWo?.job_id : (category === "job_work" ? jobId : null),
      hours: data.hours, worked_date: data.date, status: "pending",
    };
    if (category === "task" && selectedWo) insertData.routed_to_wo_id = selectedWo.work_order_id;
    if (photoUrls.length > 0) insertData.photo_urls = JSON.stringify(photoUrls);
    ...
    await notify({ supabase, type: "task_submitted", title: `Ad-hoc task: ${title.trim()}`, detail: `${formatHours(data.hours)} on ${data.date} — ${catLabel}${photoUrls.length > 0 ? ` (${photoUrls.length} photo${photoUrls.length > 1 ? "s" : ""})` : ""}`, severity: "info", freelancerId: myId, jobId: insertData.job_id, woId: selectedWo?.work_order_id, actionUrl: "/review/inbox" });
    toast.success("Task logged — pending review");
    setShowLogSheet(false); setSubmitting(false);
    router.back();
  };
  ...
  const handleStartTimer = async () => {
    ...
    if (category === "task" && selectedWo) {
      ...
      const { error } = await supabase.from("tbl_wo_time_entries").insert({ work_order_id: selectedWo.work_order_id, freelancer_id: myId, system_start_timestamp: ts, actual_start_timestamp: ts });
      if (error) { toast.error("Failed to start timer"); setSubmitting(false); return; }
      if (selectedWo.status === "Ready" || selectedWo.status === "Not-Started") await supabase.from("tbl_work_orders").update({ status: "In-Progress" }).eq("work_order_id", selectedWo.work_order_id);
      ...
      const { error } = await supabase.from("tbl_tasks").insert({ freelancer_id: myId, title: title.trim(), category, job_id: category === "job_work" ? jobId : null, started_at: new Date().toISOString(), status: "in_progress" });
  ...
  // Context for the log sheet
  const logContext = category === "task" && selectedWo
    ? { label: title || selectedWo.scope_name, sub: `${selectedWo.job_number} · ${selectedWo.description || "WO"}` }
    : { label: title || "Task", sub: category === "job_work" && selectedJob ? selectedJob.job_number : CATEGORIES.find(c => c.value === category)?.label };
  ...
  return (
    <div className="space-y-5 max-w-lg">
        ...
        <div>
          ...
          {selectedWo ? (
            <div className="flex items-center justify-between bg-surface border border-starlight-blue/30 rounded-xl px-4 py-3">
              <div><p className="text-sm font-medium text-navy">{selectedWo.scope_name}</p><p className="text-[10px] text-muted">{selectedWo.job_number} · {selectedWo.description || "—"} · {selectedWo.status}</p></div>
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                ...
                <button onClick={startScanner} className="px-4 py-3 bg-starlight-blue text-white rounded-xl flex items-center gap-2 active:bg-starlight-blue/90 shrink-0"><QrCode className="h-4 w-4" /></button>
              </div>
              {showWoPicker && filteredWos.length > 0 && (
                <div className="bg-surface border border-subtle rounded-xl shadow-lg max-h-48 overflow-y-auto">
                  {(() => {
                    ...
                    filteredWos.slice(0, 20).forEach(wo => { const key = `${wo.job_number}|${wo.scope_name}`; if (!groups[key]) groups[key] = []; groups[key].push(wo); });
                    return Object.entries(groups).map(([key, wos]) => (
                      <div key={key}>
                        <div className="px-4 py-1.5 bg-surface-dim/50 border-b border-subtle sticky top-0"><p className="text-[10px] font-bold text-navy uppercase tracking-wide">{wos[0].scope_name}</p><p className="text-[9px] text-muted font-mono">{wos[0].job_number}</p></div>
                        {wos.map(wo => (<button key={wo.work_order_id} onClick={() => { setSelectedWo(wo); setWoSearch(""); setShowWoPicker(false); setTitle(`${wo.scope_name} — ${wo.description || "WO"}`); }} className="w-full text-left pl-7 pr-4 py-2 hover:bg-surface-dim active:bg-surface-mid border-b border-subtle/50 last:border-0"><p className="text-sm text-navy">{wo.description || "—"}</p><p className="text-[10px] text-muted">{wo.status}</p></button>))}
                      </div>
                    ));
                  })()}
                </div>
      ...
      {(() => {
        ...
        const label = isWo ? (!selectedWo ? "Pick a WO first" : "Start WO Timer") : (hasActiveTask ? "Task timer already active" : "Start Timer");
        return (
      ...
      {/* Log button — opens unified LogSheet */}
      <button
        onClick={() => {
          if (category === "task" && !selectedWo) { toast.error("Pick a work order"); return; }
          if (!title.trim()) { toast.error("What did you do?"); return; }
        ...
        disabled={submitting || !title.trim() || (category === "task" && !selectedWo)}
        className="w-full py-3.5 bg-navy text-white text-sm font-semibold rounded-xl flex items-center justify-center gap-2 active:bg-navy/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
      >
      ...
      {showScanner && (
        <div className="fixed inset-0 z-50 bg-black flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center"><div id="qr-reader" className="w-full max-w-sm" /></div>
          <div className="px-4 py-4 bg-black/80 text-center"><p className="text-white/60 text-xs">Point camera at the QR code on the traveller</p></div>
        </div>
      )}
      ...
      {recentTasks.length > 0 && (
        <div className="mt-2">
          ...
          <div className="space-y-1.5">
            {recentTasks.map(t => {
              const statusStyle: Record<string, string> = { pending: "bg-starlight-amber/10 text-starlight-amber", in_progress: "bg-starlight-blue/10 text-starlight-blue", routed: "bg-starlight-blue/10 text-starlight-blue", approved_overhead: "bg-starlight-green/10 text-starlight-green", rejected: "bg-starlight-red/10 text-starlight-red" };
              return (
