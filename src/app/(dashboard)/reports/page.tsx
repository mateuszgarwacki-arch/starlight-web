  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted mt-0.5">Generate reports for jobs, scopes, and operations</p>
      </div>
        ...
        {REPORTS.map((report) => {
          ...
          return (
            <div key={report.id}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => launchReport(report.id)} disabled={!selectedJob}
                          className="flex-1 px-3 py-2 bg-starlight-blue text-white text-xs font-medium rounded-lg hover:bg-navy disabled:opacity-50 transition-colors">
                          Generate Report
                        </button>
                        <button onClick={() => { setShowJobPicker(null); setSelectedJob(null); }}
                          className="px-3 py-2 text-xs text-muted hover:bg-surface-mid rounded-lg transition-colors">Cancel</button>
                      </div>
                    </div>
                  ) : (
              ...
              {report.disabled && (
                <p className="mt-3 text-[10px] text-faint font-medium uppercase tracking-wider">Coming soon</p>
              )}
            </div>
          );
        })}
