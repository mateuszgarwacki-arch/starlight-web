...
export default function CrewPage() {
  ...
  // ============================================================
  if (loading) {
    return <div className="flex items-center justify-center h-64 text-muted text-sm animate-pulse">Loading crew...</div>;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-navy">Crew</h1>
          <p className="text-sm text-muted mt-0.5">{activeCount} active freelancers · manage people, rates, and PINs</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-base text-left">
                ...
                <th className="px-4 py-2.5 font-medium text-muted w-32">Phone</th>
                <th className="px-4 py-2.5 font-medium text-muted w-28">Role</th>
                <th className="px-4 py-2.5 font-medium text-muted w-28">Speciality</th>
                <th className="px-4 py-2.5 font-medium text-muted w-24 text-right">Day Rate</th>
                <th className="px-4 py-2.5 font-medium text-muted w-16 text-right">Hrs/Day</th>
                <th className="px-4 py-2.5 font-medium text-muted w-24 text-right">£/hr</th>
                <th className="px-4 py-2.5 font-medium text-muted w-28 text-center">PIN / Mobile</th>
                <th className="px-4 py-2.5 font-medium text-muted w-28"></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((f) => {
                ...
                return (
                  <tr key={f.freelancer_id} className={"border-t border-subtle hover:bg-navy/10/30 transition-colors " + (!isActive ? "opacity-50" : "")}>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => window.location.href = `/crew/${f.freelancer_id}`}>
                      ...
                      {f.email && <p className="text-xs text-muted">{f.email}</p>}
                    </td>
                    <td className="px-4 py-3 font-mono text-xs text-muted">{f.phone}</td>
                    <td className="px-4 py-3 text-xs text-muted">{f.role}</td>
                    <td className="px-4 py-3 text-xs text-muted">{f.speciality || "—"}</td>
                    <td className="px-4 py-3 text-right font-mono text-foreground">{f.day_rate ? formatCurrency(f.day_rate) : "—"}</td>
                    <td className="px-4 py-3 text-right font-mono text-muted">{f.standard_day_hours || "—"}</td>
                    <td className="px-4 py-3 text-right font-mono text-foreground">{hourlyRate ? formatCurrency(hourlyRate) : "—"}</td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        <span className="text-xs text-faint italic">Auth</span>
                        <button
                          onClick={() => openPinDialog(f)}
                          className="p-1 rounded transition-colors text-faint hover:text-navy hover:bg-surface-mid"
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        ...
                        <button
                          ...
                          className={"p-1.5 rounded-lg transition-colors " + (isActive ? "text-faint hover:text-starlight-red hover:bg-starlight-red/10" : "text-starlight-green hover:bg-starlight-green/10")}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-surface rounded-xl shadow-2xl w-full max-w-sm">
            ...
            <div className="px-5 py-4 space-y-4">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      autoFocus
                    />
                    <button onClick={() => setShowPin(!showPin)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-muted hover:text-muted" type="button">
                      {showPin ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                  </div>
                  <button onClick={generatePin} className="px-3 py-2 bg-surface-mid text-muted text-xs font-medium rounded-lg hover:bg-surface-hi transition-colors whitespace-nowrap" type="button">
                    Generate
                  </button>
                </div>
              </div>
              ...
              {pinResult && (
                <div className={"text-sm rounded-lg px-3 py-2 " + (pinResult.startsWith("Error") ? "text-starlight-red bg-starlight-red/10" : "text-starlight-green bg-starlight-green/10")}>
                  {pinResult}
                </div>
              )}
            </div>
            <div className="px-5 py-3 border-t border-subtle flex justify-end gap-3">
              <button onClick={() => setPinDialog(null)} className="px-4 py-2 text-sm text-muted hover:bg-surface-mid rounded-lg transition-colors">
                Close
              </button>
              <button
                onClick={savePin}
      ...
      {showAddDialog && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-surface rounded-xl shadow-2xl w-full max-w-md">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                    className="w-full px-3 py-2 border border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-starlight-blue" placeholder="07712 345 678" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-muted mb-1">Email</label>
                  <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                    className="w-full px-3 py-2 border border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-starlight-blue" placeholder="john@email.com" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" value={form.speciality} onChange={(e) => setForm({ ...form, speciality: e.target.value })}
                    className="w-full px-3 py-2 border border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-starlight-blue" placeholder="Carpenter" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-muted mb-1">Day Rate (£)</label>
                  <input type="number" value={form.day_rate} onChange={(e) => setForm({ ...form, day_rate: e.target.value })}
                    className="w-full px-3 py-2 border border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-starlight-blue" placeholder="250" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-muted mb-1">Hours/Day</label>
                  <input type="number" value={form.standard_day_hours} onChange={(e) => setForm({ ...form, standard_day_hours: e.target.value })}
                    className="w-full px-3 py-2 border border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-starlight-blue" placeholder="10" />
                </div>
                <div className="col-span-2">
            ...
            <div className="px-5 py-3 border-t border-subtle flex justify-end gap-3">
              <button onClick={() => setShowAddDialog(false)} className="px-4 py-2 text-sm text-muted hover:bg-surface-mid rounded-lg transition-colors">Cancel</button>
              <button
