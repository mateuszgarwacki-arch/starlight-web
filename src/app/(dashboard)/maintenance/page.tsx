...
function healthColor(status: string) {
  ...
  if (status === "due_soon") return "text-amber-600 bg-amber-50 border-amber-200";
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Equipment and workshop area maintenance tracking</p>
        </div>
        <button onClick={() => setShowAddAsset(true)}
          className="inline-flex items-center gap-1.5 px-4 py-2 bg-starlight-blue text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
      ...
      {assets.length > 0 && (
        <div className="flex items-center gap-4 text-xs">
          <span className="text-gray-400">{assets.length} asset{assets.length !== 1 ? "s" : ""}</span>
          {assets.filter(a => a.health_status === "overdue").length > 0 && (
            <span className="text-red-600 font-medium">{assets.filter(a => a.health_status === "overdue").length} overdue</span>
          )}
          {assets.filter(a => a.health_status === "due_soon").length > 0 && (
            <span className="text-amber-600 font-medium">{assets.filter(a => a.health_status === "due_soon").length} due soon</span>
          )}
          {assets.filter(a => a.open_flags > 0).length > 0 && (
            <span className="text-red-500 font-medium flex items-center gap-1"><Flag className="h-3 w-3" />{assets.reduce((s, a) => s + a.open_flags, 0)} open flags</span>
          )}
        </div>
      )}
      ...
      {assets.length === 0 ? (
        <div className="card px-6 py-12 text-center">
          ...
          <p className="text-gray-400 text-sm">No maintenance assets yet</p>
          <p className="text-gray-300 text-xs mt-1">Add your first piece of equipment or workshop area</p>
        </div>
      ) : (
          ...
          {assets.map(asset => (
            <div key={asset.asset_id} className="card overflow-hidden">
              ...
              <button onClick={() => toggleAsset(asset.asset_id)}
                ...
                {expandedAsset === asset.asset_id ? <ChevronDown className="h-4 w-4 text-gray-400 shrink-0" /> : <ChevronRight className="h-4 w-4 text-gray-400 shrink-0" />}
              </button>
                ...
                <div className="border-t border-gray-100 bg-gray-50/30">
                  ...
                  <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                    {editingAsset ? (
                      <div className="flex items-center gap-3 flex-1">
                        ...
                        <button onClick={() => updateAsset(asset.asset_id)} className="p-1.5 text-starlight-green hover:bg-green-50 rounded"><Save className="h-3.5 w-3.5" /></button>
                        <button onClick={() => setEditingAsset(false)} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded"><X className="h-3.5 w-3.5" /></button>
                      </div>
                    ) : (
                      <>
                        <p className="text-xs text-gray-500">{currentAsset.description || "No description"}</p>
                        <div className="flex items-center gap-1">
                          <button onClick={() => { setEditingAsset(true); setEditAssetForm({ name: currentAsset.name, location: currentAsset.location || "", description: currentAsset.description || "" }); }}
                            className="p-1.5 text-gray-400 hover:text-starlight-blue hover:bg-blue-50 rounded transition-colors" title="Edit asset">
                            <Edit2 className="h-3.5 w-3.5" />
                          </button>
                          <button onClick={() => archiveAsset(asset.asset_id)}
                            className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors" title="Archive asset">
                  ...
                  {activeTab === "checklist" && (
                    <div className="px-5 py-3 space-y-2">
                      {tasks.length === 0 ? (
                        <p className="text-xs text-gray-400 py-4 text-center">No maintenance tasks defined yet</p>
                      ) : (
                        tasks.map(task => (
                          <div key={task.task_id} className="flex items-start gap-3 py-2 border-b border-gray-100 last:border-0">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 mt-1 text-[10px] text-gray-400">
                                ...
                                <span className={task.due_status === "overdue" ? "text-red-500 font-medium" : task.due_status === "due_soon" ? "text-amber-500" : "text-green-500"}>
                            ...
                            <button onClick={() => deleteTask(task.task_id, asset.asset_id)}
                              className="p-1 text-gray-300 hover:text-red-500 transition-colors shrink-0" title="Remove task">
                      ...
                      {showAddTask ? (
                        <div className="pt-2 space-y-2 border-t border-gray-200">
                          <input value={taskForm.description} onChange={e => setTaskForm({ ...taskForm, description: e.target.value })}
                            ...
                            className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-starlight-blue" autoFocus />
                          <input value={taskForm.instructions} onChange={e => setTaskForm({ ...taskForm, instructions: e.target.value })}
                            ...
                            className="w-full px-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-starlight-blue" />
                          <div className="flex items-center gap-3">
                            <select value={taskForm.frequency} onChange={e => setTaskForm({ ...taskForm, frequency: e.target.value })}
                              className="px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-starlight-blue">
                            ...
                            <input value={taskForm.estimated_minutes} onChange={e => setTaskForm({ ...taskForm, estimated_minutes: e.target.value })}
                              type="number" placeholder="Min" className="w-16 px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-starlight-blue" />
                            <div className="flex-1" />
                            <button onClick={() => setShowAddTask(false)} className="text-xs text-gray-400 hover:text-gray-600">Cancel</button>
                            <button onClick={() => addTask(asset.asset_id)} disabled={!taskForm.description.trim()}
                              className="px-3 py-1.5 bg-starlight-blue text-white text-xs font-medium rounded-lg hover:bg-blue-700 disabled:opacity-50">Add</button>
                          </div>
                        </div>
                      ) : (
                  ...
                  {activeTab === "history" && (
                    <div className="px-5 py-3">
                      {logs.length === 0 ? (
                        <p className="text-xs text-gray-400 py-4 text-center">No maintenance sessions recorded yet</p>
                      ) : (
                        <div className="space-y-3">
                          {logs.map(log => (
                            <div key={log.log_id} className="border border-gray-100 rounded-lg p-3">
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <span className={`text-[10px] font-medium ${log.status === "completed" ? "text-green-600" : "text-amber-500"}`}>
                                ...
                                <div className="space-y-1">
                                  {log.checks.map((c, i) => (
                                    <div key={i} className="flex items-start gap-2 text-[11px]">
                                      <CheckCircle2 className={`h-3 w-3 mt-0.5 shrink-0 ${c.flagged ? "text-red-500" : "text-green-500"}`} />
                                      <span className="text-gray-600">{c.task_desc}</span>
                                      {c.note && <span className="text-gray-400 italic">— {c.note}</span>}
                                      {c.flagged && <Flag className="h-2.5 w-2.5 text-red-500 shrink-0" />}
                                    </div>
                                  ))}
                                </div>
                              )}
                              {log.notes && <p className="text-[10px] text-gray-400 mt-2 border-t border-gray-100 pt-2">{log.notes}</p>}
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  {activeTab === "flags" && (
                    <div className="px-5 py-3">
                      {flags.length === 0 ? (
                        <p className="text-xs text-gray-400 py-4 text-center flex items-center justify-center gap-1">
                          <CheckCircle2 className="h-3.5 w-3.5 text-green-400" /> No open flags
                        </p>
                      ) : (
                        <div className="space-y-2">
                          {flags.map(flag => (
                            <div key={flag.flag_id} className={`border rounded-lg p-3 ${flag.severity === "urgent" ? "border-red-200 bg-red-50/30" : flag.severity === "warning" ? "border-amber-200 bg-amber-50/30" : "border-gray-200"}`}>
                              <div className="flex items-start justify-between">
                                <div>
                                  <div className="flex items-center gap-2 mb-1">
                                    {flag.severity === "urgent" ? <AlertCircle className="h-3.5 w-3.5 text-red-500" /> :
                                     flag.severity === "warning" ? <AlertTriangle className="h-3.5 w-3.5 text-amber-500" /> :
                                     <Info className="h-3.5 w-3.5 text-blue-400" />}
                                    <span className="text-sm font-medium text-navy">{flag.title}</span>
                                    <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${flag.status === "open" ? "bg-red-100 text-red-600" : "bg-amber-100 text-amber-600"}`}>
                                      {flag.status}
                                    </span>
                                  </div>
                                  {flag.description && <p className="text-xs text-gray-500 ml-5">{flag.description}</p>}
                                  <div className="flex items-center gap-3 mt-1 ml-5 text-[10px] text-gray-400">
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                  {flag.status === "open" && (
                                    <button onClick={() => acknowledgeFlag(flag.flag_id)}
                                      className="px-2 py-1 text-[10px] text-amber-600 bg-amber-50 hover:bg-amber-100 rounded transition-colors">Acknowledge</button>
                                  )}
                                  {resolvingFlag === flag.flag_id ? (
                                    <div className="flex items-center gap-1">
                                      ...
                                      <button onClick={() => resolveFlag(flag.flag_id)}
                                        className="px-2 py-1 text-[10px] text-green-600 bg-green-50 hover:bg-green-100 rounded transition-colors">Done</button>
                                    ...
                                    <button onClick={() => setResolvingFlag(flag.flag_id)}
                                      className="px-2 py-1 text-[10px] text-green-600 bg-green-50 hover:bg-green-100 rounded transition-colors">Resolve</button>
                                  )}
                                </div>
                              </div>
                            </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setShowAddAsset(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowAddAsset(false)} className="p-1.5 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="px-5 py-4 space-y-3">
              <div>
                ...
                <input value={assetForm.name} onChange={e => setAssetForm({ ...assetForm, name: e.target.value })}
                  ...
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-starlight-blue" autoFocus />
              </div>
              <div>
            ...
            <div className="px-5 py-3 border-t border-gray-100 flex justify-end gap-3">
              <button onClick={() => setShowAddAsset(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
