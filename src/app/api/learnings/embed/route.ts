...
// Called fire-and-forget after insert; safe to call manually for backfill.
//
// Env: VOYAGE_API_KEY from https://dash.voyageai.com

const MODEL = "voyage-3-lite"; // 512 dims, generous free tier
...
export async function POST(request: NextRequest) {
  ...
  if (!apiKey) {
    ...
    return NextResponse.json({ processed: 0, disabled: ids.length, note: "VOYAGE_API_KEY not set" });
  }
  ...
  return NextResponse.json({ processed, more: pending.length === BATCH_SIZE });
}
