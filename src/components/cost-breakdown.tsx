...
export function CostBreakdown({ scopeItemId, jobId, quotedValue }: Props) {
  ...
  // Margins
  const plannedMargin = data.quoted > 0 ? data.quoted - estTotal : 0;
  const plannedMarginPct = data.quoted > 0 ? (plannedMargin / data.quoted) * 100 : 0;
  const liveMargin = data.quoted > 0 ? data.quoted - committedTotal : 0;
  const liveMarginPct = data.quoted > 0 ? (liveMargin / data.quoted) * 100 : 0;
  const actualMargin = data.quoted > 0 ? data.quoted - reconciledTotal : 0;
  const actualMarginPct = data.quoted > 0 ? (actualMargin / data.quoted) * 100 : 0;
    ...
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-4 text-xs">
          {data.quoted > 0 && (
            <span className="text-gray-500">Quoted <span className="font-semibold text-gray-800">{fmt(data.quoted)}</span></span>
          )}
          {estTotal > 0 && (
          ...
          {data.quoted > 0 && committedTotal > 0 && (
          ...
          <div className="grid grid-cols-5 gap-0 text-xs mb-4">
            ...
            {data.quoted > 0 && (
              <>
                <div className="py-2 border-t border-gray-100 font-medium text-purple-700 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-purple-400"></span> Quoted
                </div>
                <div className="py-2 border-t border-gray-100 text-right text-gray-400">—</div>
                <div className="py-2 border-t border-gray-100 text-right text-gray-400">—</div>
                <div className="py-2 border-t border-gray-100 text-right font-semibold text-gray-800">{fmt(data.quoted)}</div>
                <div className="py-2 border-t border-gray-100 text-right text-gray-400">—</div>
              </>
            )}
            ...
            <>
              ...
              <div className={`py-2 border-t border-gray-100 text-right font-semibold ${marginColor(plannedMarginPct)}`}>
                {data.quoted > 0 ? `${plannedMarginPct.toFixed(1)}%` : "—"}
              </div>
            </>
            ...
            {committedTotal > 0 && (
              <>
                ...
                <div className={`py-2 border-t border-gray-100 text-right font-semibold ${marginColor(liveMarginPct)}`}>
                  {data.quoted > 0 ? `${liveMarginPct.toFixed(1)}%` : "—"}
                </div>
              </>
            )}
            ...
            {hasReconciled && (
              <>
                ...
                <div className={`py-2 border-t border-gray-100 text-right font-semibold ${marginColor(actualMarginPct)}`}>
                  {data.quoted > 0 ? `${actualMarginPct.toFixed(1)}%` : "—"}
                </div>
              </>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 pt-3 border-t border-gray-200">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="text-[10px] text-gray-400">
                {data.quoted > 0 ? `Budget: ${fmt(data.quoted * (1 - data.targetMarginPct / 100))}` : "No quote linked"}
              </p>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="text-[10px] text-gray-400">
                {data.quoted > 0 ? fmt(hasReconciled ? actualMargin : committedTotal > 0 ? liveMargin : plannedMargin) + " profit" : "No quote linked"}
              </p>
            </div>
          </div>
        </div>
