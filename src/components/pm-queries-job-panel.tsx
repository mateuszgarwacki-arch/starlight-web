...
import { MessageCircleQuestion, Copy, Check, ChevronDown, ChevronRight, Send } from "lucide-react";
...
export function PmQueriesJobPanel({ jobId, jobName }: PmQueriesJobPanelProps) {
  ...
  const copyAllOpen = () => {
    ...
    const header = `PM Queries — ${jobName || "Job"} (${openCount} open)\n\n`;
  ...
  return (
    <div className={"card overflow-hidden " + (openCount > 0 ? "border-l-4 border-l-starlight-amber" : "")}>
      <button onClick={() => setExpanded(!expanded)}
        ...
        {openCount > 0 && (
          <span className="px-2 py-0.5 bg-starlight-amber/15 text-starlight-amber text-[10px] font-semibold rounded-full">
            {openCount} open
          </span>
        )}
        {answeredQueries.length > 0 && (
          <span className="px-2 py-0.5 bg-starlight-green/10 text-starlight-green text-[10px] font-semibold rounded-full">
            {answeredQueries.length} answered
          </span>
        )}
        <div className="flex-1" />
        {openCount > 0 && (
          <button onClick={(e) => { e.stopPropagation(); copyAllOpen(); }}
            ...
            <Copy className="h-3 w-3" /> Copy all open
          </button>
        )}
      </button>
      ...
      {expanded && (
        <div className="px-5 pb-4 space-y-2">
          {openQueries.map((q) => (
            <div key={q.query_id} className="flex items-start gap-3 px-3 py-2 bg-starlight-amber/5 border border-starlight-amber/20 rounded-lg">
                ...
                {answeringId === q.query_id ? (
                  <div className="flex items-center gap-2 mt-1.5">
                    <input type="text" value={answerText} onChange={(e) => setAnswerText(e.target.value)}
                      ...
                      autoFocus />
                    <button onClick={() => answerQuery(q.query_id)}
                      className="p-1 text-starlight-green hover:bg-starlight-green/10 rounded"><Send className="h-3 w-3" /></button>
                    <button onClick={() => { setAnsweringId(null); setAnswerText(""); }}
                      className="p-1 text-gray-400 hover:bg-gray-100 rounded"><span className="text-xs">✕</span></button>
                  </div>
                ) : (
