    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <button onClick={goToday} className="text-xs text-starlight-blue hover:text-blue-700 font-medium ml-2">
            Today
          </button>
        </div>
      ...
      {/* Calendar grid */}
      {loading ? (
        <div className="text-center py-8 text-gray-400 text-sm animate-pulse">Loading schedule...</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                <th className="text-left px-3 py-2 w-36 text-xs font-medium text-gray-500">Crew</th>
                {days.map((d, i) => (
                  <th
                    key={i}
                    className={"text-center px-1 py-2 text-xs font-medium min-w-[100px] " +
                      (isToday(d) ? "text-starlight-red" : isWeekend(d) ? "text-gray-300" : "text-gray-500")}
                  >
                    <div>{dayLabel(d)}</div>
                    {dayTotals[i] > 0 && (
                      <div className="text-[10px] font-normal text-gray-400">{dayTotals[i]} booked</div>
                    )}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {crew.map((f) => (
                <tr key={f.freelancer_id} className="border-t border-gray-100">
                  <td className="px-3 py-2">
                    <p className="text-sm font-medium text-navy">{f.freelancer_name}</p>
                    <p className="text-[10px] text-gray-400">{f.speciality || "—"}</p>
                  </td>
                  {days.map((d, i) => {
                    const ds = dateStr(d);
                    ...
                    return (
                      ...
                      >
                        {booking ? (
                          ...
                          <div className="text-gray-200 hover:text-starlight-blue transition-colors py-1">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-xs">
            ...
            <div className="px-5 py-3 border-t border-gray-100 flex justify-between">
              <div>
                {dialog.existing && (
                  <button onClick={removeBooking} className="px-3 py-2 text-xs text-starlight-red hover:bg-red-50 rounded-lg transition-colors">
                    Remove
                  </button>
                )}
              </div>
              <div className="flex gap-2">
                <button onClick={() => setDialog(null)} className="px-3 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
                  Cancel
                </button>
                <button
                  onClick={saveBooking}
                  className="px-4 py-2 bg-starlight-red text-white text-sm font-medium rounded-lg hover:bg-red-700 transition-colors"
                >
