    ...
    <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-1">
      ...
      {history.map(h => (
        <Link
          key={h.jobId}
          href={h.path}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 bg-white hover:border-starlight-blue hover:bg-starlight-blue/5 transition-colors shrink-0 group"
        >
          <span className="text-xs font-mono font-semibold text-navy">{h.jobNumber}</span>
          <span className="text-xs text-gray-500 max-w-[140px] truncate">{h.jobName}</span>
          {h.scopeName && (
            <>
              <ArrowRight className="h-3 w-3 text-gray-300 group-hover:text-starlight-blue" />
