    ...
    <div className="card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-400 font-mono">{invoices.length} invoice{invoices.length !== 1 ? "s" : ""} · {formatCurrency(totalInvoiced)}</span>
        </div>
      ...
      {!collapsed && (
        <div className="border-t border-gray-100">
          {invoices.map(inv => {
            ...
            return (
              <div key={inv.invoice_id} className="border-b border-gray-100 last:border-0">
                {/* Invoice summary row */}
                <button onClick={() => toggleExpand(inv.invoice_id)}
                  className="w-full px-5 py-2.5 flex items-center gap-4 hover:bg-gray-50/30 transition-colors text-left">
                  ...
                  <span className="text-[10px] text-gray-400 font-mono shrink-0">{inv.invoice_number || "—"}</span>
                  <span className="text-[10px] text-gray-400 shrink-0">{inv.invoice_date ? formatDate(inv.invoice_date) : ""}</span>
                ...
                {/* Expanded lines */}
                {isExp && (
                  <div className="px-5 pb-3 bg-gray-50/30">
                    {invLines.length === 0 ? (
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {invLines.map(l => {
                            ...
                            return (
                              <tr key={l.line_id} className="border-t border-gray-100 align-top">
                                ...
                                <td className="py-1.5 text-gray-600">
                                  ...
                                  {isAlloc && (
                                    <div className="bg-blue-50/50 border border-starlight-blue/20 rounded-lg px-3 py-2 mt-1.5">
                                      <div className="flex items-center justify-between mb-1.5">
                                        <p className="text-[10px] font-semibold text-navy">Allocate {formatCurrency(l.line_total || 0)}</p>
                                        <div className="flex items-center gap-2">
                                          {totalPct > 0 && <span className="text-[9px] font-mono text-gray-500">{totalPct}% · {formatCurrency((l.line_total || 0) * (100 - totalPct) / 100)} left</span>}
                                          <button onClick={() => setAllocatingLineId(null)} className="text-gray-400 hover:text-gray-600"><X className="h-3 w-3" /></button>
                                        </div>
                                      </div>
                                      {lineAllocs.map(a => (
                                        <div key={a.allocation_id} className="flex items-center gap-2 mb-1 bg-white rounded px-2 py-1 border border-gray-100">
                                          <span className="text-[10px] text-navy font-medium flex-1">{a.scope_name}</span>
                                          <span className="text-[10px] font-mono text-gray-500">{a.percentage}%</span>
                                          <span className="text-[10px] font-mono text-navy">{formatCurrency(a.allocated_amount)}</span>
                                          <button onClick={() => a.allocation_id && deleteAllocation(a.allocation_id, l.line_id)} className="p-0.5 text-gray-300 hover:text-starlight-red"><Trash2 className="h-2.5 w-2.5" /></button>
                                        </div>
                                      ))}
                                      {totalPct < 100 && (
                                        <div className="flex items-center gap-1.5 mt-1">
                                          <select id={`jip-scope-${l.line_id}`} className="flex-1 px-1.5 py-1 border border-gray-200 rounded text-[10px] bg-white focus:outline-none focus:ring-1 focus:ring-starlight-blue">
                                          ...
                                          <input id={`jip-pct-${l.line_id}`} type="number" min="1" max={100 - totalPct} defaultValue={100 - totalPct}
                                            className="w-12 px-1 py-1 border border-gray-200 rounded text-[10px] text-right font-mono focus:outline-none focus:ring-1 focus:ring-starlight-blue" />
                                          ...
                                          }} className="px-2 py-1 bg-starlight-blue text-white text-[10px] font-medium rounded hover:bg-blue-700">Add</button>
                                        </div>
                                      )}
                                    </div>
                                  )}
                                  ...
                                  {scopeItems.length > 0 ? (
                                    <div className="flex items-center gap-1">
                                      ...
                                      <button onClick={() => setAllocatingLineId(isAlloc ? null : l.line_id)}
                                        className={"p-0.5 rounded transition-colors " + (isAlloc ? "text-starlight-blue bg-blue-50" : "text-gray-400 hover:text-starlight-blue")}
